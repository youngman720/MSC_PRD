...
The site is currently scoped to **Japanese bands only**, sourced from two Japan-based music media outlets (`config/blogs.json`): [Spincoaster](https://spincoaster.com/) (indie/R&B/hip-hop leaning) and [ROCKIN'ON JAPAN](https://rockinon.com/)'s news feed (broader rock scene, including the live-house/college-band touring circuit). Both happen to use the same headline convention.

- A song qualifies if a source posted about it within the last `lookbackDays` (14, configurable) **and** it used the release-announcement phrasing both outlets share — a title containing 新曲/ニューシングル/シングル/コラボ曲/楽曲 immediately followed by a curly-quoted (“…”) song title. Both outlets use 『』 for album/EP/event names and “…” specifically for song titles, so this reliably filters out tour announcements, festival lineups, and album-only posts.
- **Nationality filter (heuristic):** both feeds cover Japanese and overseas artists together, so any post whose raw title contains one of the `excludeKeywords` in `config/blogs.json` (country/city names, "来日", etc.) is treated as non-Japanese and skipped. This is a keyword blocklist, not a real nationality check — it can both over-exclude (e.g. a Japanese artist's post that happens to mention a foreign collaborator's country) and under-exclude (an overseas artist whose post doesn't mention a recognizable place name). Tune the list as you spot misses.
- **Major-label/veteran-artist filter (manual safety net):** ROCKIN'ON JAPAN's feed in particular also covers big mainstream names (King Gnu, Ado, Mrs. GREEN APPLE, etc.) using the same release-announcement phrasing, which would otherwise pass every other filter. `excludeArtists` in `config/blogs.json` is a plain artist-name blocklist to keep those out — add to it as you spot more. (The view-velocity/subscriber-ratio metric used for the 🎓 tab below also naturally deprioritizes most such acts since their huge subscriber base absorbs any view spike — but it's not foolproof: if the YouTube search fallback attaches the wrong channel to a song, an artist can get a misleadingly high ratio. `excludeArtists` catches what the ratio misses.)
